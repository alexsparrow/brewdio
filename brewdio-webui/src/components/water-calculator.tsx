...
const INITIAL_STAGES: WaterStage[] = [
  {
    id: "mash",
    label: "Mash Tun",
    stageType: "mash",
...
const WaterCalculator: React.FC = () => {
  ...
  const [stages, setStages] = useState<WaterStage[]>(INITIAL_STAGES);
          ...
          <svg viewBox={`0 0 ${SVG_WIDTH} ${SVG_HEIGHT}`} className="w-full h-auto">
            <defs>
                ...
                <linearGradient id="liquidGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                 ...
                 {data.stages.map((stage: CalculatedWaterStage) => (
            ...
            {/* --- Render Loop --- */}
            {data.stages.map((stage: CalculatedWaterStage, i: number) => {
              const xPos = 50 + (i * 200);
              const tankTopY = BASELINE_Y - CYLINDER_HEIGHT;

              const shape = stageTypeToShape(stage.stageType);
              const isConical = shape === 'conical';
              ...
              return (
                <g key={stage.id}>
                  ...
                  <g transform={`translate(${xPos}, ${tankTopY})`}>
                      ...
                      {/* 1. Tank Shell (Glass) */}
                      <path
                        d={getTankPath(shape, TANK_WIDTH, CYLINDER_HEIGHT)}
                        fill="#1e293b"
                        fillOpacity="0.5"
                        stroke="#475569"
                        strokeWidth="3"
