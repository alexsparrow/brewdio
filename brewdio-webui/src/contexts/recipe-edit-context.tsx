...
interface RecipeEditContextValue {
  ...
  update: (draftFn: (draft: { recipe: RecipeType; updatedAt?: number }) => void) => Promise<void>;
...
export function RecipeEditProvider({
  ...
  const update = useCallback(
    async (draftFn: (draft: { recipe: RecipeType; updatedAt?: number }) => void) => {
      if (type === "recipe") {
        const current = await getBackend().getRecipe(id);
        if (!current) return;
        const draft = { recipe: structuredClone(current.recipe) } as { recipe: RecipeType; updatedAt?: number };
        draftFn(draft);
        await getBackend().updateRecipe(id, draft.recipe.name, draft.recipe);
        ...
        draftFn(draft);
        await updateBatchImperative(id, batchDoc.name, {
          ...batchDoc,
          recipe: draft.recipe,
