...
import { type ChatInit, type LanguageModel, type ToolSet } from "ai";
...
export function useChat(model: LanguageModel, tools: ToolSet, options?: CustomChatOptions) {
  ...
  if (!transportRef.current) {
    transportRef.current = new CustomChatTransport(model, tools);
  }
